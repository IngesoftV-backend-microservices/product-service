...
# Copiar JAR
COPY --from=build --chown=appuser:appuser /app/target/*.jar product-service.jar
...
# Punto de entrada
ENTRYPOINT ["sh", "-c", "java $JAVA_OPTS -Dspring.profiles.active=$SPRING_PROFILES_ACTIVE -Dserver.port=$SERVER_PORT -jar product-service.jar"]